...
import { Modal } from '../components/HomeChat/Modal';
...
function HomeChat() {
  ...
  useEffect(() => {
    /* 
      ...
      2. Get chats data
    */
    apiClient.get('/users', Authorization).then((res) => dispatch(setUserData(res.data)));
    apiClient.get('/chats', Authorization).then((res) => dispatch(setChatsData(res.data.chats)));
  }, []);
  ...
  useEffect(() => {
    if (ref.current) {
      ...
      */
      socketClient.on('chats', () => {
        apiClient
          .get('/chats', Authorization)
  ...
  const handleSendMsg = async () => {
    if (msgEntry !== '') {
      ...
      /*
        ...
        1. Send message
      */
      await apiClient.post(`/chats/${selectedChat}`, { message: msgEntry }, Authorization);
    } else {
      /* TODO: 
        1. Show error notification
      */
      NotificationFailure('Hubo un error al mandar el mensaje, intenta de nuevo por favor.');
    }
  };
